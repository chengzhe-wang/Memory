...
function chargementImage(){
	for(i = 0; i<mesCartes.length; i++){
		mesCartes[i].visible = true;
	}
	setTimeout(chargement, 10);
	
}

function chargement(){
	for(j = 0; j<mesCartes.length; j++){ // J'avais un problème de chargement d'image, quand je cliquais, l'image de s'affichait pas tout le temps. J'ai donc changé dans le constructeur (au final j'ai créé des fonctions) pour que les cartes soient visibles initialement puis je fais une boucle pour les mettre de dos juste avant d'afficher les cartes.
    	mesCartes[j].visible = false; //Finalement ça ne règle que partiellement le problème, au bout d'un certain temps (environ 8 secondes après le chargement de la page ***le temps varie selon les navigateurs***), j'ai encore des problèmes de chargement de cartes.
    } //Je pense que c'est surtout après avoir refresh plusieurs fois la page que les problèmes de chargement apparaissent, quand j'ouvre memory pour la première fois il n'y a aucun problème
}